...
It does **not** run an authoritative DNS server. To serve hostname → IP mappings for a zone (e.g., `bartschnet.de`), you need an external authoritative DNS server such as CoreDNS, BIND, or PowerDNS.
...
1. **CoreDNS** remains the authoritative server for the internal zone (e.g., `bartschnet.de`)
...
bartschnet.de {
    hosts {
        192.168.1.10 server.bartschnet.de
        192.168.1.20 nas.bartschnet.de
        192.168.1.30 proxy.bartschnet.de
        fallthrough
    }
    log
}
...
```corefile
bartschnet.de {
    file /etc/coredns/zones/bartschnet.de.zone
    log
}
```

Zone file (`bartschnet.de.zone`):
```zone
$ORIGIN bartschnet.de.
$TTL 3600
...
| Port | `53` |
| Match Domains | `bartschnet.de` |
| Primary | No (unless it should handle all queries) |
| Search Domains | Optional: enable so `server` resolves as `server.bartschnet.de` |
...
- **Wildcard subdomain** resolution (e.g., `*.bartschnet.de → 192.168.1.1`)
...
```
Peer requests: server.bartschnet.de
       ↓
NetBird local DNS resolver on peer
       ↓ (checks match-domain rules)
"bartschnet.de" → forward to 100.x.y.z:53 (CoreDNS)
       ↓
...
2. Verify CoreDNS is reachable from peers: `ping <coredns-ip>` then `dig @<coredns-ip> server.bartschnet.de`
3. Check `netbird status` on peer to see which DNS config was received

**CoreDNS not responding:**
```bash
# Test directly
dig @localhost server.bartschnet.de
# Check container logs
docker logs coredns
```

**Match domain not triggering:**
- Ensure the domain in NetBird exactly matches (e.g., `bartschnet.de` not `bartschnet.de.`)
- NetBird match domains are suffix-matched: `bartschnet.de` also matches `sub.bartschnet.de`
